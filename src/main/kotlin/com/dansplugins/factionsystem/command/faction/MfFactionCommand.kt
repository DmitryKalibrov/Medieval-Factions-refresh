...
import com.dansplugins.factionsystem.command.faction.admin.MfFactionAdminTeamCommand
...
import com.dansplugins.factionsystem.command.faction.guide.MfFactionGuideCommand
...
import com.dansplugins.factionsystem.command.faction.kingroll.MfFactionKingRollCommand
...
import com.dansplugins.factionsystem.command.faction.team.MfFactionTeamCommand
import com.dansplugins.factionsystem.command.faction.team.info.MfFactionTeamInfoCommand
import com.dansplugins.factionsystem.command.faction.team.join.MfFactionTeamJoinCommand
import com.dansplugins.factionsystem.command.faction.team.list.MfFactionTeamListCommand
...
class MfFactionCommand(private val plugin: MedievalFactions) : CommandExecutor, TabCompleter {
    ...
    private val factionGuideCommand = MfFactionGuideCommand(plugin)
    ...
    private val factionTeamCommand = MfFactionTeamCommand(plugin)
    private val factionTeamJoinCommand = MfFactionTeamJoinCommand(plugin)
    private val factionTeamInfoCommand = MfFactionTeamInfoCommand(plugin)
    private val factionTeamListCommand = MfFactionTeamListCommand(plugin)
    private val factionAdminTeamCommand = MfFactionAdminTeamCommand(plugin)
    private val factionKingRollCommand = MfFactionKingRollCommand(plugin)

    private val guideAliases = listOf("guide", "гайд", "руководство")
    ...
    private val teamJoinAliases = listOf("teamjoin", "jointeam", "команда")
    private val teamInfoAliases = listOf("teaminfo", "командаинфо")
    private val teamListAliases = listOf("teamlist", "команды")

    private val subcommands = guideAliases +
        createAliases +
        claimAliases +
        lawAliases +
        allyAliases +
        ...
        teamInfoAliases +
        teamListAliases +
        listOf("team", "admin", "kingroll")

    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        return when (args.firstOrNull()?.lowercase()) {
            "team" -> factionTeamCommand.onCommand(sender, command, label, args.drop(1).toTypedArray())
            "admin" -> if (args.getOrNull(1)?.lowercase() == "team") factionAdminTeamCommand.onCommand(sender, command, label, args.drop(2).toTypedArray()) else false
            "kingroll" -> factionKingRollCommand.onCommand(sender, command, label, args.drop(1).toTypedArray())
            in guideAliases -> factionGuideCommand.onCommand(sender, command, label, args.drop(1).toTypedArray())
            ...
            in teamJoinAliases -> factionTeamJoinCommand.onCommand(sender, command, label, args.drop(1).toTypedArray())
            in teamInfoAliases -> factionTeamInfoCommand.onCommand(sender, command, label, args.drop(1).toTypedArray())
            in teamListAliases -> factionTeamListCommand.onCommand(sender, command, label, args.drop(1).toTypedArray())
            else -> {
                sender.sendMessage("${SpigotChatColor.of("#CD7F32")}Система средневековых фракций в Minecraft!")
                sender.sendMessage("")
                sender.sendMessage("${SpigotChatColor.of("#D2691E")}Здесь вы можете:")
                sender.sendMessage("${SpigotChatColor.of("#B87333")}• Создавать и управлять фракциями")
                sender.sendMessage("${SpigotChatColor.of("#B87333")}• Захватывать территории")
                sender.sendMessage("${SpigotChatColor.of("#B87333")}• Вести войны и заключать союзы")
                sender.sendMessage("${SpigotChatColor.of("#B87333")}• Строить экономику и политику")
                sender.sendMessage("")
                sender.sendMessage("${SpigotChatColor.of("#FF6B35")}📚 Для подробного изучения используйте: /faction guide")
                true
            }
        }
    }
    ...
    override fun onTabComplete(
        ...
        args.size == 1 -> subcommands.filter { it.startsWith(args[0].lowercase()) }
            ...
            in guideAliases -> factionGuideCommand.onTabComplete(sender, command, label, args.drop(1).toTypedArray())
            ...
            in teamJoinAliases -> factionTeamJoinCommand.onTabComplete(sender, command, label, args.drop(1).toTypedArray())
            in teamInfoAliases -> factionTeamInfoCommand.onTabComplete(sender, command, label, args.drop(1).toTypedArray())
            in teamListAliases -> factionTeamListCommand.onTabComplete(sender, command, label, args.drop(1).toTypedArray())
            else -> emptyList()
        }
    }
}
