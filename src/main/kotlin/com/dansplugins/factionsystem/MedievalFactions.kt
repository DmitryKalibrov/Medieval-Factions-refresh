...
import com.dansplugins.factionsystem.command.economy.MfEconomyCommand
...
import com.dansplugins.factionsystem.economy.SimpleMfEconomyRepository
...
import com.dansplugins.factionsystem.team.MfTeamService
...
class MedievalFactions : JavaPlugin() {
    ...
    lateinit var teamService: MfTeamService
    lateinit var economyService: MfEconomyService
    val gson = Gson()
    ...
    override fun onEnable() {
        ...
        val economyRepository = SimpleMfEconomyRepository(this)
        ...
        val economyService = MfEconomyService(this, economyRepository)
        this.economyService = economyService
        val ui = MfUi(
            ChatColor.of(config.getString("ui.palette.primary") ?: "#EADBC8"),
            ChatColor.of(config.getString("ui.palette.accent") ?: "#A8C8A6"),
            ChatColor.of(config.getString("ui.palette.info") ?: "#A9C3E8"),
            ChatColor.of(config.getString("ui.palette.success") ?: "#9AC59A"),
            ChatColor.of(config.getString("ui.palette.warning") ?: "#E6B980"),
            ChatColor.of(config.getString("ui.palette.danger") ?: "#E38B8B"),
            ChatColor.of(config.getString("ui.palette.muted") ?: "#C9C9C9")
        )
        ...
        teamService = MfTeamService(this)
        setupRpkLockService()
        ...
        getCommand("economy")?.setExecutor(MfEconomyCommand(this))

        // Team-based power tick (rarer, scaled by team size)
        server.scheduler.scheduleSyncRepeatingTask(this, {
            ...
            server.scheduler.runTaskAsynchronously(
                ...
                Runnable {
                    ...
                    // Apply team bonus less frequently
                    try {
                        applyTeamPowerBonus(onlineMfPlayerIds)
                    } catch (e: Exception) {
                        logger.log(SEVERE, "Failed to apply team power bonus: ${e.message}", e)
                    }
                }
            )
    ...
    internal fun onPowerCycle(
        ...
        // Отправляем уведомления о королевском бонусе
        sendKingBonusNotifications(onlineMfPlayerIds, originalOnlinePlayerPower, newOnlinePlayerPower, initialPower)
    ...
    /**
     * Отправляет уведомления о королевском бонусе
     */
    private fun sendKingBonusNotifications(
        onlineMfPlayerIds: List<MfPlayerId>,
        originalOnlinePlayerPower: Map<MfPlayerId, Double>,
        newOnlinePlayerPower: Map<MfPlayerId, Double>,
        initialPower: Double
    ) {
        server.scheduler.runTask(
            this,
            Runnable {
                onlineMfPlayerIds.forEach { playerId ->
                    val team = teamService.getPlayerTeam(playerId)
                    val isKing = team?.let { teamService.getKing(it) == playerId } ?: false

                    if (isKing && (team == TeamColor.RED || team == TeamColor.BLUE)) {
                        val originalPower = originalOnlinePlayerPower[playerId] ?: initialPower
                        val newPower = newOnlinePlayerPower[playerId] ?: initialPower
                        val totalIncrease = floor(newPower).roundToInt() - floor(originalPower).roundToInt()

                        if (totalIncrease > 0) {
                            // Рассчитываем стандартное увеличение (без бонуса короля)
                            val standardIncrease = calculateStandardPowerIncrease(originalPower)
                            val kingBonus = totalIncrease - standardIncrease

                            if (kingBonus > 0) {
                                val player = server.getPlayer(playerId.value) ?: return@forEach
                                player.sendMessage("$GREEN${language["KingPowerBonus", kingBonus.toString()]}")
                            }
                        }
                    }
                }
    ...
    /**
     * Рассчитывает стандартное увеличение силы (без бонуса короля)
     */
    private fun calculateStandardPowerIncrease(currentPower: Double): Int {
        val maxPower = config.getDouble("players.maxPower")
        val minPower = config.getDouble("players.minPower")

        // Простая формула: 1% от оставшейся силы до максимума
        val standardIncrease = (maxPower - currentPower) * 0.01

        return floor(standardIncrease).roundToInt().coerceAtLeast(0)
    }

    // Adds small extra power increments based on team population, called inside power cycle task
    private fun applyTeamPowerBonus(onlineMfPlayerIds: List<MfPlayerId>) {
        val teamService = teamService
        val playerService = services.playerService
        // Compute bonus factor: sqrt of team size / 100, capped
        val redSize = teamService.getTeamSize(com.dansplugins.factionsystem.team.TeamColor.RED)
        val blueSize = teamService.getTeamSize(com.dansplugins.factionsystem.team.TeamColor.BLUE)
        val yellowSize = teamService.getTeamSize(com.dansplugins.factionsystem.team.TeamColor.YELLOW)
        val factors = mapOf(
            com.dansplugins.factionsystem.team.TeamColor.RED to (kotlin.math.sqrt(redSize.toDouble()) / 100.0),
            com.dansplugins.factionsystem.team.TeamColor.BLUE to (kotlin.math.sqrt(blueSize.toDouble()) / 100.0),
            com.dansplugins.factionsystem.team.TeamColor.YELLOW to (kotlin.math.sqrt(yellowSize.toDouble()) / 100.0)
        )
        onlineMfPlayerIds.forEach { pid ->
            val team = teamService.getPlayerTeam(pid) ?: return@forEach
            val mf = playerService.getPlayer(pid) ?: return@forEach
            val factor = factors[team] ?: 0.0
            if (factor <= 0.0) return@forEach
            val newPower = (mf.power + factor).coerceAtMost(config.getDouble("players.maxPower"))
            playerService.save(mf.copy(power = newPower))
        }
    }
